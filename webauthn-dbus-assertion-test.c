...
#define BUS_NAME    "org.gnome.WebAuthn"
#define OBJECT_PATH "/org/gnome/WebAuthn"
#define INTERFACE   "org.gnome.WebAuthn.Authenticator"
    ...
    size_t signature_len)
{
    ...
    if (!cbor_is_type(kty, CBOR_TYPE_UNSIGNED) ||
        kty->uint_value != 2)
        goto cleanup;

    if (!cbor_is_type(alg, CBOR_TYPE_NEGATIVE) ||
        alg->int_value != -7)
        goto cleanup;

    if (!cbor_is_type(crv, CBOR_TYPE_UNSIGNED) ||
        crv->uint_value != 1)
        goto cleanup;

    if (!cbor_is_type(x, CBOR_TYPE_BYTES) ||
        !cbor_is_type(y, CBOR_TYPE_BYTES) ||
        x->bytes.length != 32 ||
        y->bytes.length != 32)
        goto cleanup;
    ...
    uint8_t client_data_hash[32])
{
    ...
    cbor_map_put(
        rp,
        cbor_new_text("id"),
        cbor_new_text("example.com"));

    cbor_map_put(
        rp,
        cbor_new_text("name"),
        cbor_new_text("GNOME WebAuthn Test"));
    ...
    cbor_map_put(
        user,
        cbor_new_text("name"),
        cbor_new_text("test@example.com"));

    cbor_map_put(
        user,
        cbor_new_text("displayName"),
        cbor_new_text("GNOME WebAuthn Test User"));
...
int main(void)
{
    ...
    printf("GNOME WebAuthn D-Bus assertion test\n");
    printf("===================================\n");
    ...
    printf("[1] Connecting to session D-Bus...\n");
    ...
    if (!bus) {
        fprintf(
            stderr,
            "Unable to connect to session bus: %s\n",
            error ? error->message : "unknown error");
    ...
    printf("[2] Building MakeCredential request...\n");

    if (build_makecredential_request(
            &input,
            &input_len,
            client_data_hash) != 0) {
        ...
        fprintf(
            stderr,
            "Failed to build MakeCredential request\n");
    ...
    printf(
        "    Request length: %zu bytes\n",
        input_len);

    printf(
        "[3] Sending MakeCredential through D-Bus...\n");

    printf(
        "    >>> Fingerprint verification should now be requested <<<\n");
    ...
    if (dbus_process(
        ...
        fprintf(
            stderr,
            "MakeCredential D-Bus request failed\n");
    ...
    printf(
        "    D-Bus response length: %zu bytes\n",
        output_len);
    ...
    if (decode_response(
        ...
        fprintf(
            stderr,
            "MakeCredential response decode failed\n");
    ...
    if (!auth_data ||
        ...
        fprintf(
            stderr,
            "MakeCredential authData missing\n");
    ...
    printf(
        "    authData length: %zu bytes\n",
        auth_data->bytes.length);

    if (extract_credential_data(
            auth_data,
            &credential_id,
            &credential_id_len,
            &cose_key) != 0) {
        ...
        fprintf(
            stderr,
            "Could not extract credential data\n");
    ...
    printf("[4] Building GetAssertion request...\n");

    if (build_getassertion_request(
            client_data_hash,
            credential_id,
            credential_id_len,
            &input,
        ...
        fprintf(
            stderr,
            "Failed to build GetAssertion request\n");
    ...
    printf(
        "[5] Sending GetAssertion through D-Bus...\n");
    ...
    if (dbus_process(
        ...
        fprintf(
            stderr,
            "GetAssertion D-Bus request failed\n");
    ...
    if (decode_response(
        ...
        fprintf(
            stderr,
            "GetAssertion response decode failed\n");
    ...
    printf(
        "[6] GetAssertion response decoded successfully.\n");

    CborValue *assert_auth_data =
        cbor_map_get_uint(response, 2);

    CborValue *signature =
        cbor_map_get_uint(response, 3);
    ...
    if (!assert_auth_data ||
        ...
        fprintf(
            stderr,
            "GetAssertion response missing required fields\n");
    ...
    printf("[7] Verifying ECDSA signature...\n");

    size_t signed_data_len =
        assert_auth_data->bytes.length + 32;
    ...
    if (!signed_data) {
        ...
        fprintf(
            stderr,
            "Unable to allocate signed data\n");
    ...
    printf("\n");
    printf("===================================\n");
    printf("D-Bus WebAuthn assertion test: PASS\n");
    printf("===================================\n");
