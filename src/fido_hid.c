...
static int handle_cbor(uint32_t cid, const uint8_t *data, size_t len)
{
    ...
    /*
     * User verification is deliberately not handled by the HID transport.
     * authenticator_process() owns CTAP UV policy so that D-Bus and HID
     * requests follow exactly the same path and authenticatorData flags
     * reflect the actual per-operation verification state.
     */
